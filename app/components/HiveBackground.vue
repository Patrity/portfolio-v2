...
function drawGrid(ctx: CanvasRenderingContext2D, grid: Grid, dpr: number, now: number): void {
  ...
  for (let i = 0; i < cells.length; i++) {
    const cell = cells[i]!
    const e = cell.e
    const breathe = cell.idle ? 0.5 + 0.5 * Math.sin(breathePhase + cell.ph) : 0
...
function startHive(canvas: HTMLCanvasElement): (() => void) | null {
  ...
  function rebuild(): void {
    ...
    if (reduced && grid) drawGrid(ctx!, grid, dpr, performance.now())
  }
